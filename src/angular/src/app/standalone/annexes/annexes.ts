...
@Component({
  ...
  imports: [MatTabsModule, FormsModule, MatButtonToggleModule, MatIconModule, MatTableModule, MatExpansionModule, AnnexeBlocComponent],
...
export class Annexes {
  ...
  /* `mat-accordion multi` émet l'état voulu plutôt que de demander une bascule : plus de
     `has() ? delete : add`, on applique directement ce que le panneau annonce. */
  protected setSection(i: number, expanded: boolean): void {
    this.openSections.update((set) => {
      const next = new Set(set);
      if (expanded) next.add(i);
      else next.delete(i);
      return next;
    });
  }
  ...
  protected readonly legsColumns = ['code', 'fait', 'jambes'];
