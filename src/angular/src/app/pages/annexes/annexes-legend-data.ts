...
const LEGS_TONE = { bg: 'rgba(67,56,202,0.16)', fg: 'var(--ink-code)' };
const LEGS_OPT_TONE = { bg: 'rgba(180,83,9,0.16)', fg: 'var(--ink-warn)' };
...
export const LEG_GROUP: LegendGroup = {
  ...
  rows: [
    { kind: 'code', label: 'BUY', ...LEGS_TONE, meaning: "Achat de titres · nature TRADE · Le décaissement se déduit de quantité × prix + frais + taxe : aucune jambe d'espèces.", where: '1 opération' },
    { kind: 'code', label: 'SELL', ...LEGS_TONE, meaning: "Vente de titres · nature TRADE · L'encaissement se déduit de quantité × prix − frais − taxe.", where: '1 opération' },
    { kind: 'code', label: 'BUYOPT', ...LEGS_TONE, meaning: "Souscription par exercice de droits · nature CORPORATE · Comptabilisée comme un achat : le paiement se déduit des champs.", where: '1 opération' },
    { kind: 'code', label: 'SPLIT', ...LEGS_TONE, meaning: 'Division ou regroupement · nature CORPORATE · La parité fait tout ; une opération par compte détenteur, sous la même transaction.', where: '1 par compte' },
    { kind: 'code', label: 'SPINOFF', ...LEGS_TONE, meaning: 'Scission · nature CORPORATE · Le coût de revient se répartit entre ligne mère et ligne reçue.', where: '1 opération' },
    { kind: 'code', label: 'MERGER', ...LEGS_TONE, meaning: "Fusion-absorption · nature CORPORATE · Le coût passe intégralement sur la ligne de l'absorbante.", where: '1 opération' },
    { kind: 'code', label: 'DIV', ...LEGS_TONE, meaning: "Dividende en numéraire · nature CORPORATE · Vit dans corporateActions : ni transactions ni cashOperations, le net se déduit de amount − tax.", where: '0 (champs à plat)' },
    { kind: 'code', label: 'DIVOPT', ...LEGS_TONE, meaning: 'Dividende optionnel sans rompu · nature CORPORATE · La parité tombe juste : titres seuls, coût total inchangé.', where: '1 opération' },
    { kind: 'code', label: 'DIVOPT', ...LEGS_TONE, meaning: "Dividende optionnel avec soulte · nature CORPORATE · La soulte ne se déduit de rien : jambe de titres + jambe d'espèces (sense = soulte).", where: '2 opérations' },
    { kind: 'code', label: 'TOUT', ...LEGS_TONE, meaning: "Transfert sortant hors périmètre · nature TRANSFER · Le compte d'arrivée n'est pas suivi : rien à apparier.", where: '1 opération' },
    { kind: 'code', label: 'TOUT + TIN', ...LEGS_TONE, meaning: "Transfert entre deux comptes suivis · nature TRANSFER · Départ et arrivée, éventuellement à des dates différentes ; supprimer une jambe supprime l'autre.", where: '2 opérations' },
    { kind: 'code', label: 'TIN', ...LEGS_TONE, meaning: 'Entrée depuis un établissement non suivi · nature TRANSFER · La saisie fournit le coût de base, qui devient le PRU d\'entrée.', where: '1 opération' },
    { kind: 'code', label: 'TRANSFER', ...LEGS_TONE, meaning: "Virement d'espèces entre comptes · nature TRANSFER · Débit à la source, crédit à l'arrivée : deux poches distinctes, aucune ne se déduit de l'autre.", where: '2 opérations' },
    { kind: 'code', label: 'DEPOSIT', ...LEGS_TONE, meaning: "Versement d'espèces · nature CASHFLOW · Fait simple : une seule poche touchée.", where: '1 opération' },
    { kind: 'code', label: 'WITHDRAW', ...LEGS_TONE, meaning: "Retrait d'espèces · nature CASHFLOW · Identique au versement, sens inverse.", where: '1 opération' },
    { kind: 'code', label: 'INTEREST', ...LEGS_TONE, meaning: 'Intérêts sur liquidités · nature CASHFLOW · Aucun titre en jeu.', where: '1 opération' },
    { kind: 'code', label: 'FEE', ...LEGS_TONE, meaning: "Frais et droits de garde · nature CASHFLOW · À distinguer des frais portés par une opération sur titres.", where: '1 opération' },
    { kind: 'code', label: 'LENDING', ...LEGS_TONE, meaning: "Revenus de prêt de titres · nature CASHFLOW · Seule la commission est enregistrée ; le prêt lui-même n'est pas suivi.", where: '1 opération' },
    { kind: 'code', label: 'REFUND', ...LEGS_TONE, meaning: 'Remboursement · nature CASHFLOW · La charge remboursée reste inscrite à sa date.', where: '1 opération' },
  ],
};
...
export const OPT_GROUP: LegendGroup = {
  ...
  rows: [
    { kind: 'code', label: 'DIVOPT', ...LEGS_OPT_TONE, meaning: "Dividende optionnel — Une seule jambe quand la parité tombe juste ; deux dès qu'un rompu est réglé en espèces — la soulte ne se déduit d'aucun autre champ. Un journal ancien porte la soulte dans cashBalance de la jambe de titres, forme encore lue telle quelle.", where: '1 ou 2' },
    { kind: 'code', label: 'TOUT', ...LEGS_OPT_TONE, meaning: "Transfert sortant de titres — Une jambe si le compte d'arrivée n'est pas suivi ; deux dès que la contrepartie est un compte du journal, la jambe TIN étant alors appariée et supprimée avec elle.", where: '1 ou 2' },
    { kind: 'code', label: 'SPLIT', ...LEGS_OPT_TONE, meaning: "Division ou regroupement — Autant d'opérations que de comptes portant le titre : une décision de l'émetteur touche toutes les lignes, sous une même transaction.", where: '1 par compte' },
    { kind: 'code', label: 'SPINOFF', ...LEGS_OPT_TONE, meaning: 'Scission — Une jambe pour la ligne mère ; une seconde si la ligne créée entre sur un autre compte ou si une soulte règle les rompus.', where: '1, parfois 2' },
    { kind: 'code', label: 'MERGER', ...LEGS_OPT_TONE, meaning: "Fusion-absorption — Une jambe suffit quand la ligne reçue reste sur le même compte ; deux si une soulte en espèces complète la parité d'échange.", where: '1, parfois 2' },
    { kind: 'code', label: 'FEE', ...LEGS_OPT_TONE, meaning: 'Frais — Une jambe de trésorerie seule, ou deux quand les frais accompagnent un mouvement de titres — la nature CASHFLOW couvre les deux journaux.', where: '1 ou 2' },
    { kind: 'code', label: 'LENDING', ...LEGS_OPT_TONE, meaning: 'Prêt de titres — La commission seule, ou deux jambes si le prêt est suivi côté titres — le type couvre les deux journaux.', where: '1 ou 2' },
  ],
};
